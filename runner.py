...
Features:
- Pre-loading of molecules before protein etching
...
async def main():
    """Main function with molecules pre-loaded before protein etching"""
    try:
        # Parse arguments
        ...
        # =============== FIRST PHASE: LOAD AND PREPROCESS MOLECULES ===============
        
        # Initialize PSICHIC model first
        bt.logging.info("Initializing PSICHIC model...")
        model = EnhancedPsichicWrapper()
        ...
        # Pre-load all molecules first
        bt.logging.info(f"Pre-loading {len(smiles_list)} molecules...")
        preloaded_dict = model.initialize_smiles(smiles_list)
        bt.logging.info("Molecule pre-loading complete")
        
        # =============== SECOND PHASE: GET AND PROCESS PROTEINS ===============
        
        # Get proteins after molecules are loaded
        bt.logging.info("Fetching challenge proteins...")
        ...
        bt.logging.info("Fetching protein sequences...")
        for code in target_codes + antitarget_codes:
            ...
            if sequence:
                ...
                bt.logging.info(f"Fetched sequence for {code}: {len(sequence)} amino acids")
            else:
                bt.logging.error(f"Failed to fetch sequence for {code}")
        
        # Now that molecules are pre-loaded, preprocess all proteins in batch
        bt.logging.info(f"Preprocessing {len(protein_sequences)} proteins in batch...")
        model.initialize_proteins_batch(protein_sequences)
        bt.logging.info("Protein preprocessing complete")
        ...
        for protein_code in target_codes + antitarget_codes:
            ...
            if not sequence:
                bt.logging.warning(f"Skipping protein {protein_code}: sequence not found")
                continue
        ...
        bt.logging.info("Beginning protein processing with pre-loaded molecules...")
        for protein in all_proteins:
            ...
            if not result_df.empty:
                ...
                bt.logging.info(f"Successfully processed protein {protein['protein_code']}")
            else:
                bt.logging.warning(f"No results for protein {protein['protein_code']}")

...
class SAVILookup:
    ...
    def lookup_smiles_batch(self, molecule_names):
        ...
        """
        self.logger.info(f"Looking up SMILES for {len(molecule_names)} molecules in batch")
        ...
        try:
            ...
            self.logger.info(f"Found SMILES for {len(result)}/{len(molecule_names)} molecules ({len(result)/len(molecule_names)*100:.1f}%)")
            return result
...
async def run_validator_check(
    ...
    try:
        ...
        # Look up SMILES in batch
        print("Looking up SMILES in database...")
        savi = SAVILookup("savi_lookup.db")
        smiles_dict = savi.lookup_smiles_batch(molecule_names)
        ...
        for data in molecule_data:
            ...
            if molecule_name in smiles_dict:
        ...
        for data in valid_molecules:
            ...
            if uid in all_target_scores and uid in all_antitarget_scores:
                ...
                weighted_diff_score = (target_weight * target_score_avg) - (1.0 * antitarget_score_avg)
        ...
        if results:
            ...
            print(f"Molecules found in database: {len(smiles_dict)}/{len(molecule_names)}")
...
def parse_args():
    ...
    parser.add_argument("--min-heavy-atoms", type=int, default=24,
                        help="Minimum number of heavy atoms for molecules (default: 17)")
    parser.add_argument("--target-weight", type=float, default=1.0,
                        help="Weight for target protein affinity scores (default: 1.0)")
    parser.add_argument("--antitarget-weight", type=float, default=1.5,
                        help="Weight for antitarget protein affinity scores (default: 1.5)")
